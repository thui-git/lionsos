...
static ssize_t sock_write(const void *buf, size_t len, int fd) {
    ...
    return socket_config->tcp_socket_write(socket_handle, buf, len);
}

static ssize_t sock_read(void *buf, size_t len, int fd) {
    int socket_handle = fd_socket[fd];
    return socket_config->tcp_socket_recv(socket_handle, buf, len);
}
...
static long sys_setsockopt(va_list ap) {
    ...
    //TODO: default -ENOPROTOOPT; handle specific options as needed
    return 0;
}
...
static long sys_sendto(va_list ap) {
    ...
    return socket_config->tcp_socket_write(fd_socket[sockfd], buf, len);
}
...
static long sys_recvfrom(va_list ap) {
    ...
    int read = socket_config->tcp_socket_recv(socket_handle, buf, len);
...
static long sys_ppoll(va_list ap) {
    ...
    for (nfds_t i = 0; i < nfds; i++) {
        ...
        if (fd_socket[fd] != -1) {
            ...
            if (socket_config->tcp_socket_err(sock)) {
